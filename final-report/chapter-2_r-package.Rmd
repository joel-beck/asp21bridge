...
1. The parameters of the Full Conditionals of section \@ref(full-conditional) are calculated, depending on the inputted data.

2. For each parameter to simulate $i$, a initial matrix $\boldsymbol{R}_i$, where $\boldsymbol{R}_i \in \mathbb{R}^{m+1 \, \times \, r}$ with the number of simulations $m$ and the dimension of the parameters $r$ is created. 
The first entry of $\boldsymbol{R}_i$ is always the inputted starting value for the parameter (`beta_start`, `gamma_start`, `xi_start`, `tau_start`). 
