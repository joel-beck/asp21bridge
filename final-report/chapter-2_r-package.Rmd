...
Thus, the resulting values can be interpreted as samples from the joint Posterior Distribution (as described in chapter \@ref(math)), which cannot be obtained analytically.
...
The `mcmc_ridge()` function is structured in a modular manner; it contains various internal helper functions for different tasks that are collected in the `mcmc_ridge_helpers` file in order to maintain a concise and readable code structure.
...
The `validate_input()` function handles these aspects and extracts all necessary input parameters that are required by the sampling process.
...
Next, the `includes_intercept()` function checks both design matrices for intercept columns, while the `add_intercept()` adds those for the sampling procedure, if necessary.
...
Since this part lies right at the heart of the `mcmc_ridge()` function, it is explained in more detail below.
...
In this case, the sampling matrices for $\boldsymbol{\beta}$ and $\boldsymbol{\gamma}$ are assigned the names `location` and `scale`, respectively, which plays nicely into the existing code base of the `lmls` package and in particular for adapting the `summary()` output to the `mcmc_ridge()` estimates.
...
The following paragraph explains the iterative Gibbs updates of the `mcmc_ridge()` function in a stepwise fashion.
...
Update the value of `acc_count_scale` if the proposed value for $\boldsymbol{\gamma}$ was accepted:
    ...
    ```{r, eval=FALSE}
    gamma_list <- mh_gamma(
      beta_samples[i, ], gamma_samples[i - 1, ],
      xi_samples[i - 1, ], ...
    )
