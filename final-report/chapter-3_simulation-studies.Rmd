...
pacman::p_load(dplyr, readr, ggplot2, tidyr, patchwork, magrittr)
library(asp21bridge)
```
...
1. Evaluating the **quality** and **robustness** of the sampler.
    ...
    Section \@ref(corr) induces correlation into the underlying data, which can potentially be challenging for estimation.
    Just as in sections \@ref(redundant) and \@ref(outcome), the variation in the input parameters is therefore focused on the function argument `m` or alternatively the combination of `X`, `Z` and `y`.
...
Since the resulting simulation studies serve such diverse purposes (e.g. diagnostic vs. exploratory), they demand for different approaches in the simulation settings, the implementation as well as the analysis and presentation of the results.
...
In order to keep the analysis in this chapter succinct, there will be almost no code included.
It it worth noting though that all `R Scripts` used for the simulations are contained in the `simulation-studies` folder of the `asp21bridge` package.
...
As stated in part \@ref(guide), each regressor variable is independently sampled from a normal distribution and the outcome variable is simulated based on the correctly specified location-scale regression model introduced in chapter \@ref(math).
...
Hence, many of the simulation studies contained in this chapter intend to imitate data configurations that might be closer to data found in the real world.
We start by inducing correlation among the predictor variables.
Further, the `mcmc_ridge()` performance is compared to the Maximum Likelihood based `lmls()` estimates and the unpenalized Bayesian `mcmc()` sampler.

### Simulation Setting {.unlisted .unnumbered}

Briefly stated, we simulate covariate data from a three dimensional multivariate normal distribution with three different values for the pairwise correlation $\rho$.
Then, we fit the three models `mcmc_ridge()`, `mcmc()` and `lmls()`, where the number of simulations was set to $10.000$ for the Bayesian Samplers.
...
All three models perform equally well for each correlation structure:
...
Due to this similarity between the three models, the remaining analysis of this simulation study is dedicated to the `mcmc_ridge()` model only.
...
Although the acceptance rates of the proposed values are in acceptable ranges for a random walk proposal (between $35$% and $40$%), the chains are strongly correlated and did not fully converge even after $10.000$ iterations.
...
Overall, these results emphasize the sampler's stability in presence of correlated covariates.
...
It is worth noting that the results are partially affected by the standardization of the covariates before fitting the models:
The standardization generally leads to a stabilized fit for all three of them.
...
This simulation study analyzes the effect of the sample size `n` on the Posterior Means for the coefficient vectors $\boldsymbol{\beta}$ and $\boldsymbol{\gamma}$. 
There are two main goals of this simulation study: 
On the one hand, we want to investigate whether the posterior means of large samples are closer to the true values than the posterior means of small samples. 
On the other hand, we want to analyze whether the `mcmc_ridge()` penalty affects the location of the posterior means.

### Simulation Setting {.unlisted .unnumbered}

For the analysis purposes of this simulation study, the sample size `n` varies between $30$ and $500$. 
There are in total six different scenarios.

In the first step, the n-dimensional covariates $\mathbf{x}_1$, $\mathbf{x}_2$, $\mathbf{ z}_1$ and $\mathbf{ z}_2$ are all sampled from a normal distribution with variance $1$. 
After that, all of these four vectors are standardized to have zero mean and unit variance. 
Then the n-dimensional vector $\mathbf{ y}$ is sampled based on a linear model with an error term drawn from a standard normal distribution. 
The `mcmc_ridge()` function is applied afterwards and provides estimates for the Posterior Means of $\boldsymbol{\beta}$ and $\boldsymbol{\gamma}$.
The underlying true coefficients as well as all details of the simulation setting can be found in the appendix. 

To make the results more reliable, above procedure is repeated $100$ times for each of the six different sample sizes. 
The performance of the estimates is then measured for each of the $\boldsymbol{\beta}$ and $\boldsymbol{\gamma}$ coefficients by the mean values of the Posterior Means and the *MAE* over the $100$ repetitions.  
...
None of the parameters seems to be biased even for a sample size of `n` $= 30$.
...
The right plot of Figure \@ref(fig:sample-size) points out that the posterior means of all six coefficients have mean absolute errors of quite the same magnitude. 
In addition, for increasing sample sizes, the *MAE* of the Posterior Means converges to zero for all coefficients. 
...
Similar to the simulation study from section \@ref(corr), this part investigates the sampler's behaviour in presence of strong *pairwise* correlation among the covariates.
...
The statistical theory of (Ridge) penalization suggests, that the coefficient estimates from the `mcmc_ridge()` function are affected by the shrinkage effect induced by the coefficient's prior distributions in presence of a high number of correlated covariates.
Thus, the magnitude of the estimated $\boldsymbol{\beta}$ vector should be smaller compared to models without a regularization component.
...
Since the purpose of this study is to demonstrate the Ridge regularization and not primarily the accuracy of the models, we used the covariates on their original scale for generating $\mathbf{y}$ and standardized them afterwards before fitting the models.
Thus, the true coefficient values cannot be immediately compared to the estimates without further transformations, such that conclusions about the model *quality* are not valid.
...
We start with answering the first question:
For each of the three models and each coefficient pair the absolute value of the coefficient corresponding to the relevant regressor is larger than the magnitude of the paired quantity.
...
Note also, that the intercept term is typically excluded from the penalization, which is the reason why the estimates of $\beta_0$ are of approximately equal size for all three models.
...
For this simulation study standardization makes a huge difference, such that coefficient estimates are a lot more consistent for all $9$ scenarios, when the models are fit with standardized regressor columns.
...
For all three outcome distributions, the estimates of the `mcmc_ridge()` function are consistently closest to zero compared to the unpenalized models.
...
The large overall magnitude of the intercept term is induced by mean-centering all covariates between data generation and model fitting.
...
Analogously, section \@ref(gamma) illustrates the dependence of the Full Conditional distribution of $\boldsymbol{\gamma}$ on $\xi^2$, which in turn depends on the hyperparameters $a_{\xi}$ and $b_{\xi}$.
...
These connections are reflected in the `mcmc_ridge()` sampler by the iterative sampling procedure which is discussed in great detail in the previous sections \@ref(mcmc-ridge) and \@ref(mh).
...
Section \@ref(hyper) is dedicated to the impact of $a_\tau$, $b_\tau$, $a_\xi$ and $b_\xi$ on the accuracy of the Posterior Mean estimates for $\boldsymbol{\beta}$ and $\boldsymbol{\gamma}$, whereas section \@ref(hyper-penalty) examines their effect on the Ridge Penalty that is induced by the Inverse-Gamma prior distributions.


### Simulation Setting {.unlisted .unnumbered}

The design matrices are generated by normal distributions. 
Moreover, $\boldsymbol{\beta}$ is sampled more reliably from a closed form multivariate normal distribution by the `mcmc_ridge()` function (rather than by an additional Metropolis-Hastings step) in order to distinguish effects from the hyperparameters and effects caused by general estimation variability.
A more detailed description of the simulation setting is listed in the Appendix.
...
For a better overview, the dashed line displays the linear trend of all estimate deviations.
...
It can be observed, that the intercept estimates in each plot show the largest deviations from their true value for $\boldsymbol{\beta}$, whereas the largest deviations can be found for $\gamma_1$ among the $\boldsymbol{\gamma}$ estimates.
...
In contrast, deviations of the $\boldsymbol{\gamma}$ estimates in the right panel are fairly significant, especially for $\gamma_1$.
...
A larger value for $b_\tau$ leads to larger values of $\tau^2$, which in turn affects the Full Conditional distribution of $\boldsymbol{\beta}$ and, thus, potentially magnifies the absolute deviation of the corresponding estimates from their true values. 
Increasing $a_\tau$ causes the opposite effect. 
This numerically observable effect, however, is hidden by the overall small deviation in the upper left plot of Figure \@ref(fig:hyppar). 

It is remarkable, that the deviation of ${\boldsymbol{\beta}}$ estimates is smallest when $a_\tau, b_\tau \in \left\{100, 200\right\}$. 
...
The lowest deviations of ${\boldsymbol{\gamma}}$ estimates are connected to values $a_\tau, b_\tau \in \{1, 50, 100\}$, where $1$ likely results from randomness, since for $a_\tau \leq 1$ wide variations of absolute deviations are observable again.
...
In general, one obtains no significant differences in deviations of $\boldsymbol{\gamma}$ along $b_{\xi}$, but lower deviations for larger values of $a_{\xi}$, where especially lots of randomness occurs in the deviations of $\gamma_1$. 
Therefore, the impact of $a_{\xi}$ and $b_{\xi}$ on the scale parameter is overshadowed by the randomness induced by the Metropolis-Hastings algorithm. 
The even wider variations exclusively for $a_{\xi} \leq 1$ can again be observed.
...
The larger variability of deviations for $a_{\xi} \leq 1$ are once more observable here. 
In contrast, the randomness observable for scale estimates does not show up for location estimates anymore.
...
This section relates the values of $a_\tau$, $b_\tau$, $a_\xi$ and $b_\xi$ to the observed Penalty on the coefficient estimates for $\boldsymbol{\beta}$ and $\boldsymbol{\gamma}$, where we exclude $\beta_0$ and $\gamma_0$ from the analysis since the intercept terms are not penalized in the Bayesian Ridge Regression model.
...
Thus, we construct grids of value pairs for $a_\tau$ and $b_\tau$ for the prior distribution of $\boldsymbol{\beta}$ and pairs of $a_\xi$ and $b_\xi$ for the prior distribution of $\boldsymbol{\gamma}$.
...
As usual, more details about the simulation setting are provided in the Appendix.
...
For comparison, the unpenalized Euclidean norms of the true data generating coefficient values are given by $\left \Vert \boldsymbol{\beta} \right \Vert^2 = 68$ and $\left \Vert \boldsymbol{\gamma} \right \Vert^2 = 18$.

Starting with the top panel, there are almost no patterns observable.
...
However, we have to keep the ranges of the color scales in mind.
...
This finding is somewhat surprising, since e.g. the theoretical Posterior Mean of $\tau^2$ differs wildly throughout the chosen combinations, suggesting a heavily varying prior variance of $\boldsymbol{\beta}$ and therefore different *potentials* for larger absolute values.
...
While the pattern of this reduction is not super clear in case of $\boldsymbol{\beta}$ and could potentially be due to randomness, the values of $\left \Vert \boldsymbol{\gamma} \right \Vert^2$ are consistently lower for $a_\xi \geq 64$ and $b_\xi \leq 4$.
This combination leads to low Posterior Means for $\xi^2$, which again induces a lower / more informative prior variance $\tau^2$.

The results from this simulation study are difficult to connect to their hypothesized immediate effect based on the underlying mathematical model and appear in some cases even counter-intuitive.
This clearly illustrates that tweaking parameters at one end in a Bayesian model of moderate complexity might not affect the model as intended originally.
...
This final section briefly summarizes the results that were obtained in this chapter.
Recall that we aimed to investigate two main aspects by means of the six simulation studies we analyzed and simultaneously provide some general guidance, in which cases the `asp21bridge` package can be a useful alternative to competing approaches.

On the one hand the sampler's *performance* was examined in order to gain some basic credibility.
As seen in sections \@ref(corr), \@ref(n) and \@ref(hyper), the `mcmc_ridge()` function provides solid estimates across a wide range of circumstances.
If the data is standardized before fitting the model, the results are very robust towards various disadvantageous conditions of the collected data set, such as highly correlated covariates, a small sample size, non-normal response variables and even an uninformed choice of the hyperparameters of the underlying hierarchical Bayesian model.

The location parameter $\boldsymbol{\beta}$ in particular is estimated with high precision throughout, with slightly more variability for the scale parameter estimates.
One exception might be the lacking accuracy of the intercept estimates $\beta_0$ and $\gamma_0$.
If, for some reason, estimation of the intercept terms is of primary interest, alternative estimation procedures should be considered.

The second goal of this chapter was to explore the *penalty* effect that is induced by the Prior distributions in the Bayesian framework, which is likely the major reason why one might use the `asp21bridge` package in the first place.
Sections \@ref(redundant) and \@ref(outcome) emphasized the shrinkage effect on all coefficients except the intercept terms in comparison with unpenalized models.
There, the scenario of a high number of (potentially strongly correlated) predictor variables was covered, which is inherently prone to overfitting and, thus, a strong candidate for using regularized methods.
Further, the effect of a Ridge ($L_2$) penalty in contrast to a LASSO ($L_1$) penalty was highlighted.

Unlike in the standard linear model with Ridge penalty or in a non-hierarchical Bayesian model with Gaussian Prior distribution, there is unfortunately not a single parameter $\lambda$ nor a single prior variance parameter that we can choose, to manually control the *strength* of the Ridge penalty.
The results of section \@ref(hyper-penalty) indicated that there indeed exists a connection from the hyperparameter choice to the penalty strength. 
However, a significant impact could only be found for very specific combinations which, in addition, do not naturally arise from intuition about the model structure.
Hence, most input choices of the `mcmc_ridge()` function lead to a moderate penalty effect and therefore a limited bias on the coefficient estimates as well as a moderate reduction in variance.
