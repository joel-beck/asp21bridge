...
There are in total six different szenarios.
...
Therefore, the impact of $a_{\xi}$ and $b_{\xi}$ on the scale paremeter is overshadowed by the randomness induced by the Metropolis-Hastings algorithm. 
...
While the pattern of this reduction is not super clear in cae of $\boldsymbol{\beta}$ and could potentially be due to randomness, the values of $\left \Vert \boldsymbol{\gamma} \right \Vert^2$ are consistenly lower for $a_\xi \geq 64$ and $b_\xi \leq 4$.
This combination leads to low Posterior Means for $\xi^2$, which again induces a lower / more informative prior variance $\tau^2$.

The results from this simulation study are difficult to connect to their hypothesized immediate effect based on the underlying mathematical model and appear in some cases even counterintuitive.
