...
For the analysis purposes of this simulation study, the sample size `n` varies between 30 and 500. There are in total six different szenarios.

In the first step, the `n`-dimensional covariates $x_1$, $x_2$, $z_1$ and $z_2$ are all sampled from a normal distribution with variance 1. After that, all of these four vectors are standardized to have zero mean and unit variance. Then the `n`-dimensional vector $y$ is sampled based on a linear model with an error term drawn from a standard normal distribution. The underlying true coefficients can be found in the appendix. The `mcmc_ridge()` function is applied afterwords and provides estimates for the Posterior Means of $\boldsymbol{\beta}$ and $\boldsymbol{\gamma}$.

To make the results more reliable, above procedure is repeated 100 times for each of the six different sample sizes. The performance of the estimates is then measured for each of the $\boldsymbol{\beta}$ and $\boldsymbol{\gamma}$ coefficients by the mean Posterior Mean and the *MAE* over the 100 repetitions.  
...
The right plot of Figure \@ref(fig:sample-size) points out that the posterior means of all six coefficients have mean absolute errors of quite the same magnitude. In addition, for increasing sample sizes, the *MAE* of the Posterior Means tend to zero for all coefficients. 
...
The design matrices are generated by normal distributions. Moreover, $\beta$ is sampled out of the closed form in the `mcmc_ridge()` function to see direct effects, that do not diminish through a *Metropolis-Hastings step*.
...
It can be observed, that the intercept estimates in each plot show the largest deviations from their true value for $\beta$. Whereas for all $\gamma$ estimates, the largest deviations can be found for $\gamma_1$.
...
Anyway, our sampler produces the lowest deviation of ${\boldsymbol{\gamma}}$ estimates  for $a_\tau, b_\tau \in \{1, 50, 100\}$, where $1$ is chosen by coincidence for $a_\tau$ here, since wide variations for $a_\tau \leq 1$ of absolute deviations are observable again.
...
In general, one obtains no significant differences in deviations of $\gamma$ along $b_{\xi}$, but lower deviations for larger values of $a_{\xi}$, where especially lots of randomness occurs in the deviations of $\gamma_1$. Therefore, the impact of $a_{\xi}$ and $b_{\xi}$ on the *scale* intercepts is overshadowed by the randomness induced by the *Metropolis-Hastings algorithm*. 
The even wider variations exclusively for $a_{\xi} \leq 1$ can again be obtained.

The sampler exhibits the best results for the *scale* estimates for $a_\xi = 2$ and $b_\xi \in \left\{-1, 50\right\}$. 
...
The linear trends in the lower left part of Figure \@ref(fig:hyppar) for values of $a_{\xi}$ and $b_{\xi}$ coming off inferior. 
The wider variations of deviations for $a_{\xi} \leq 1$ are once more observable here. 
...
Shortly noted, the acceptance rates of the Metropolis Hastings algorithm for sampling ${\boldsymbol{\gamma}}$ are always between $0.21$ and $0.39$. 
For the value range of $a_{\tau}$ and $b_{\tau}$, no distinct pattern is observable in this regard. 
With growing values of $a_{\xi}$ however, acceptance rates are more likely to decrease. Whereas growing values of $b_{\xi}$ causes the opposite more likely. 
