...
rownames(mean_of_variances_within) <- c(
  "beta_0", "beta_1", "beta_2", "gamma_0",
  "gamma_1", "gamma_2"
)
...
for (r in 1:rep) {
  ...
  for (j in 1:num_sim_length) {
    samples <- mcmc_ridge(
      m = model, X = X, Z = Z, y = y, num_sim = array_num_sim[j],
    ...
    variances_within_samples_array[, j, r] <- c(
      diag(var(samples$sampling_matrices$beta_samples)),
