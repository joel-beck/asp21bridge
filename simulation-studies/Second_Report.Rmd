...
```{r, corr-plot-single, echo=FALSE, out.width="100%", fig.cap="Comparison of Correlation Structures - one Simulation", fig.pos="t"}
...
```{r, corr-plot-many, echo=FALSE, out.width="100%", fig.cap="Comparison of Correlation Structures - 50 Simulations", fig.pos="t"}
...
- The design matrix $\mathbf{Z} = \begin{pmatrix} \mathbf{1}_{n} & \mathbf{x}_1 & \mathbf{x}_2 & \mathbf{z}_3 \end{pmatrix}^T$ contains the additional regressor variable $\mathbf{x}_3 \stackrel{ iid}{ \sim} \mathcal{t}_{10}$, which is independently sampled from all other columns.
...
```{r, outcome-plot-single, echo=FALSE, out.width="100%", fig.cap="Comparison of Outcome Distributions - one Simulation", fig.pos="t"}
...
Interestingly, the $\boldsymbol{\gamma}$ vector is estimated very well in the latter case with more deviations when $y$ is based on the $t$ distribution.
...
The means as well as empirical confidence intervals (analogous to section \@ref(corr)) are plotted in figure \@ref(fig:outcome-plot-many).

```{r, outcome-plot-many, echo=FALSE, out.width="100%", fig.cap="Comparison of Outcome Distributions - 50 Simulations", fig.pos="t"}
outcome_dist_list$plot_many_sims
```

Estimates of the bias and the standard error of the Posterior Mean / Maximum Likelihood estimates can be more distinctly compared by their numerical values provided by tables \@ref(tab:bias) and \@ref(tab:se).
