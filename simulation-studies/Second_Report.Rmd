...
```{r, include=FALSE}
pacman::p_load(dplyr, readr, ggplot2, tidyr, kableExtra)
library(asp21bridge)
```
...
For each covariance structure the three models `mcmc_ridge()`, `mcmc()` and `lmls()` were fitted.
...
2. The `mcmc_ridge()` sampler (as well as the `mcmc()` and `lmls()` functions) are very robust towards correlated data and perform extremely well. 
In particular, all three approaches (visually) provide close to unbiased estimates.

3. The variability among the $\boldsymbol{\beta}$ estimates is almost nonexistent, such that results from a single simulation are already reliable and representative.
