...
In the past, we have been sampling data by our `mcmc_ridge` function without having a closer look on the effect of the hyperparameters `a_tau`, `b_tau`, `a_xi` and `b_xi`. However, they affect the full conditionals of $\tau^2$ and $\xi^2$, which are Inverse Gamma distributed, directly as stated here again for convenience:
<!--  -->
$$
\begin{aligned}
...
\end{aligned}
$$
In a next step mean and variance of the full conditional of $\boldsymbol{\beta}$ following a normal distribution are assumed to be positively affected by $\boldsymbol{\tau^2}$:
<!--  -->
$$
\begin{aligned}
...
\end{aligned}
$$
while $\boldsymbol{\xi^2}$ directly affects the full conditional of $\boldsymbol{\beta}$:
<!--  -->
$$
\begin{aligned}
...
- The design matrix $\mathbf{X} = \begin{pmatrix} \mathbf{x}_1 & \mathbf{x}_2 \end{pmatrix}$ is simulated from a two dimensional normal distribution $\mathcal{N}_{2} \left( \boldsymbol{\mu}, \mathbf{\sigma^2 I} \right)$ with mean vector $\boldsymbol{\mu} = \begin{pmatrix} 1 & 2 \end{pmatrix}^T$ and unit variance $\boldsymbol{\sigma^2_1 = \sigma^2_2 = 1}$. Same as the design matrix $\mathbf{Z} = \begin{pmatrix} \mathbf{z}_1 & \mathbf{z}_2 \end{pmatrix}$ with mean vector $\boldsymbol{\mu} = \begin{pmatrix} 5 & 3 \end{pmatrix}^T$ also having a unit variance.
...
- For sampling the location parameter, the full posterior, that follows a normal distribution is chosen (`mcmc_ridge(..., mh_location = FALSE)` is used). Therefore, at least the location estimate is more directly affected by the hyperparameters.

- For simulating the influence of the hyperparameters, nine different values are chosen for $\boldsymbol{a_{\tau}} = \boldsymbol{b_{_\tau}} =  \boldsymbol{a_{\xi}} = \boldsymbol{b_{xi}} = \begin{pmatrix} -1 & 0 & 0.5 & 1 & 2 & 10 & 50 & 100 & 200 \end{pmatrix}$. Since for statistical properties like the mean of an Inverse Gamma distribution $\boldsymbol{\frac{ b}{ a-1}}$, values for $\boldsymbol{a > 0}$ are required, particular attention is focused on larger values. Anyway, it is an aim to inspect the performance of the sampler for values $\boldsymbol{\le 1}$ as well.

## Simulation Results

Figure \@ref(fig:hyppar-tau-values) displays the absolute deviation of the posterior mean estimates of the `mcmc_ridge()` sampler and the true parameters with the stated different values for `a_tau` and `b_tau`. For each estimate, the posterior mean averages over 1000 simulations of the `mcmc_ridge()` sampler. Note, that the plot splits parameters by location and scale, according to the relationship mentioned above. For a better overview, the dotted line displays the linear trend of all estimate's deviations.

The scaling of the $x$ - axis is transformed by a pseudo logarithm to clearly visualize the deviations between $-1$ and $10$, which would not be possible on original scales. Since $-1$ and $0$ are also part of the hyperparameter values, the `pseudo_log_trans()` of the `scales` package is applied logarithmizing only positive values. It can be obtained, that the intercept estimates in each plot show the largest deviations from their true value. In the upper panel, however, deviations of `beta` estimates are absolutely small and can be considered to not affect the performance of the sampler unambiguously. On the contrary, deviations of the `gamma` estimates in the lower panel are absolutely remarkable, especially for `gamma_0`.

```{r, hyppar-tau-values, echo=FALSE, out.width="100%", fig.cap="Comparison of the absolute deviations over beta and gamma parameters- a_tau", fig.pos="t"}
hyperparameters_list$tau_dev_plot
```

The functional chain that applies to the `beta` estimates can be described by the effect of the mean of the inverse gamma distribution on $\boldsymbol{\tau^2}$. A larger value for $\boldsymbol{b_\tau}$ leads to larger values of $\boldsymbol{\tau^2}$ which are again affecting the full posterior parameters of $\boldsymbol{\beta}$ and by this the absolute deviation of the `beta` estimates from their true value to be larger. $\boldsymbol{b_\tau}$ causes the opposite effect. Nevertheless, this outcome is not clearly visible in the upper panel and is assumed to be surpassed by the overall small deviation. It is remarkable, that the deviation of both estimates is the smallest for $\boldsymbol{a_\tau, b_\tau \in [50, 100]}$. For values of $\boldsymbol{a_\tau \le 1}$ one obtains wide variances of absolute deviations, since the posterior mean required values larger than one.
In the lower panel, there is no clear effect of $\boldsymbol{\tau^2}$ and its parameters. Rooted in no direct effect of $\boldsymbol{\tau^2}$ on $\boldsymbol{\gamma}$ according to our underlying mathematical model, one observes cross-effects through the sampling procedure of our `mcmc_ridge()` sampler, where the full posterior $\boldsymbol{f( \boldsymbol{\gamma} \mid \cdot )}$ depends on $\boldsymbol{\beta}$. Anyway, our sampler performs the best for $\boldsymbol{a_\tau, b_\tau \in \{0.5, 200\}}$, where $0.5$ is chosen by coincidence here, since wide variations for $\boldsymbol{a_\tau \le 1}$ of absolute deviations are observable again.
