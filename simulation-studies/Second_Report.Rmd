...
$$
<!--  -->
while $\xi^2$ directly affects the full conditional of $\boldsymbol{\beta}$:
<!--  -->
$$
\begin{aligned}
...
$$
<!--  -->
Simulating data samples for different hyperparameters values is therefore inevitable for the whole model. 
...
Figures \@ref(fig:hyppar-tau-beta) and \@ref(fig:hyppar-tau-gamma) display the absolute deviations of the posterior mean estimates from the true parameters with the stated different values for `a_tau` and `b_tau`. 
For each estimate, the posterior mean averages over 1000 simulations of the `mcmc_ridge()` sampler. 
Note, that the plot splits parameters by location and scale, according to the relationship mentioned above. 
...
In the upper panel, however, deviations of `beta` estimates are small in absolute value and can be considered to not affect the performance of the sampler unambiguously. 
On the contrary, deviations of the `gamma` estimates in the lower panel are absolutely remarkable, especially for `gamma_0`.

```{r, hyppar-tau-beta, echo=FALSE, out.width="100%", fig.cap="Comparison of the absolute deviations of beta parameters", fig.pos="t"}
hyperparameters_list$p1 / hyperparameters_list$p3
```


```{r, hyppar-tau-gamma, echo=FALSE, out.width="100%", fig.cap="Comparison of the absolute deviations of gamma parameters", fig.pos="t"}
hyperparameters_list$p2 / hyperparameters_list$p4
```
...
Nevertheless, this outcome is not clearly visible in the upper panel and is assumed to be surpassed by the overall small deviation. 

It is remarkable, that the deviation of both estimates is the smallest for $a_\tau, b_\tau \in \left\{50, 100\right\}$. 
For values of $a_\tau \leq 1$ one obtains wide variances of absolute deviations, since the posterior mean required values larger than one.

In the lower panel, there is no clear effect of $\tau^2$ and its parameters. 
...
Anyway, our sampler performs the best for $a_\tau, b_\tau \in \{0.5, 200\}$, where $0.5$ is chosen by coincidence here, since wide variations for $a_\tau \leq 1$ of absolute deviations are observable again.
[EFFECT OF XI ON BETA, KREUZEFFEKTE?] 
